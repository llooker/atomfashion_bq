view: session_purchase_facts {
  derived_table: {
    ...
    from (
      ...
      JOIN @{schema}.atom_order_items on atom_order_items.created_at = atom_events.created_at
      --JOIN @{schema}.inventory_items  AS inventory_items ON inventory_items.id = order_items.inventory_item_id
